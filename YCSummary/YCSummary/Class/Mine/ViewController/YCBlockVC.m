...
-(NSArray *)dataArray{
    if (!_dataArray) {
        _dataArray=@[@"声明Block类型变量并赋值：",@"使用typedef简化Block",@"Block作为C函数参数",@"Block作为OC函数参数",@"Block内访问局部变量",@"Block内访问__block修饰的局部变量",@"Block内访问static修饰的局部变量"];
    }
    return _dataArray;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    switch (indexPath.section) {
        ...
        case 5:
            ...
            break;
        case 6:
            [self visitlocalVariableblockWithAStatic];
            break;
        default:
            break;
    }
...
-(void)__blockvisitlocalVariableblock{
   __block int global=100;
    ...
    /*在局部变量前使用下划线下划线block修饰,在Block中可以直接修改局部变量,
     ...
     __block修改变量：global->__forwarding->global
     __Block_byref_global_0结构体内部地址和外部变量global是同一地址
    */
}
#pragma mark -访问局部变量的block
-(void)visitlocalVariableblockWithAStatic{
    static int global=100;
    void(^localBlcok)()=^{
        DebugLog(@"==%d",global);//==101
    };
    global = 101;
    localBlcok();
}
